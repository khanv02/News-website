document.addEventListener("DOMContentLoaded", function () {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // refreshScrollTriggers: gọi sau khi intro hoàn tất.
  // QUAN TRỌNG: sau đó dispatch "nw:page-ready" để section2.js mới được init.
  // Điều này đảm bảo section2 tính toán vị trí CHÍNH XÁC sau khi layout ổn định.
  // ────────────────────────────────────────────────────────────────────────────
  function refreshScrollTriggers() {
    ...
    requestAnimationFrame(function () {
      ...
      // Dispatch sau một frame nữa để refresh hoàn thành
      requestAnimationFrame(signalPageReady);
    });
  }

  function signalPageReady() {
    // Đánh dấu window để fallback checker của section2 biết
  ...
  function Phase_two() {
    ...
    // Zoom
    tl.to(introText, {
      x: m.moveX, y: m.moveY, scale: m.scaleTarget,
      duration: C.moveDuration, ease: "power2.inOut", force3D: true
    }, "introMoveStart");

    // Wipe
    tl.to(wipeLayer, {
      "--nw-wipe-right": "0%",
      duration: C.wipeDuration, ease: "power3.inOut"
    }, "introMoveStart");
  ...
  if (card) {
    ...
    card.addEventListener("mousemove", function (e) {
      ...
      gsap.to(card, {
        rotationY: nx * 5, rotationX: -ny * 5, z: -10,
